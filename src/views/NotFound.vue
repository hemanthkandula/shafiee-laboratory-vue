<template>


    <div itemscope="" class=" error-page error-404 browser-chrome is-withMagicUnderlines v-glyph v-glyph--m2 is-js" data-gr-c-s-loaded="true">
        <div class="site-main" id="container">
            <div class="butterBar butterBar--error"></div>
            <div class="surface">
                <div id="prerendered" class="screenContent">
                    <section class="container u-marginAuto u-maxWidth1000 u-marginTop60 u-xs-marginTop15">
                        <div class="row u-lineHeightTightest">
                            <h1 class="col u-size5of12 u-sm-size12of12 u-xs-paddingLeft12 u-sm-fontSize150 u-fontSize200 u-fontWeightBold">404</h1>
                            <h2 class="col u-size7of12 u-sm-size12of12 u-xs-fontSize36 u-fontSize75 u-letterSpacingTighter u-textColorLight u-paddingTop70 u-sm-paddingTop0 u-paddingLeft0 u-sm-paddingLeft20">We couldn’t find this page you're looking for.Head back to
                                <router-link exact to="/">
                                    Home</router-link>

                                .</h2>
                        </div>
                        <div>
                            <p class="u-marginTop20 u-marginBottom0 u-xs-marginBottom10 u-padingRight20 u-textColorLight">Maybe it’s out there, somewhere...</p>
                            <!--<p class="u-padingRight20 u-textColorLight">    We can't find the page you're looking for. Head back to-->
                                <!--<a class=" link&#45;&#45;underline" href="/">homepage</a>.-->
                            <!--</p>-->
                        </div>
                    </section>
                </div>
            </div>
        </div>
    </div>



</template>

<script>
    export default {
        name: "NotFound"
    }
</script>

<!--<style scoped>-->
    <!--/*! CSS Used from: https://cdn-static-1.medium.com/_/fp/css/main-branding-base._lSoV8jnvyEzg3I0Y9SGBw.css */-->
    <!--body{-->
        <!--margin:0;-->
    <!--}-->
    <!--section{-->
        <!--display:block;-->
    <!--}-->
    <!--a{-->
        <!--background-color:transparent;-->
    <!--}-->
    <!--a:active,a:hover{-->
        <!--outline:0;-->
    <!--}-->
    <!--h1{-->
        <!--font-size:2em;-->
        <!--margin:.67em 0;-->
    <!--}-->
    <!--body{-->
        <!--font-family:medium-content-sans-serif-font,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen,Ubuntu,Cantarell,"Open Sans","Helvetica Neue",sans-serif;-->
        <!--letter-spacing:0;-->
        <!--font-weight:400;-->
        <!--font-style:normal;-->
        <!--text-rendering:optimizeLegibility;-->
        <!-- -webkit-font-smoothing:antialiased;-->
        <!-- -moz-osx-font-smoothing:grayscale;-->
        <!-- -moz-font-feature-settings:"liga" on;-->
        <!--color:rgba(0,0,0,.84);-->
        <!--font-size:20px;-->
        <!--line-height:1.4;-->
    <!--}-->
    <!--h1,h2{-->
        <!--font-family:medium-content-sans-serif-font,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen,Ubuntu,Cantarell,"Open Sans","Helvetica Neue",sans-serif;-->
        <!--letter-spacing:0;-->
        <!--font-weight:700;-->
        <!--font-style:normal;-->
    <!--}-->
    <!--a{-->
        <!--color:inherit;-->
        <!--text-decoration:none;-->
    <!--}-->
    <!--a{-->
        <!-- -webkit-tap-highlight-color:transparent;-->
    <!--}-->
    <!--body{-->
        <!--overflow-x:hidden;-->
    <!--}-->
    <!--h1,h2,p{-->
        <!--margin:0;-->
    <!--}-->
    <!--p{-->
        <!--margin-bottom:30px;-->
    <!--}-->
    <!--@media print{-->
        <!--h2{-->
            <!--page-break-after:avoid;-->
            <!--page-break-inside:avoid;-->
        <!--}-->
    <!--}-->
    <!--@media print and (color){-->
        <!--*{-->
            <!-- -webkit-print-color-adjust:exact;-->
            <!--print-color-adjust:exact;-->
        <!--}-->
    <!--}-->
    <!--.u-marginBottom0{-->
        <!--margin-bottom:0!important;-->
    <!--}-->
    <!--.u-marginTop20{-->
        <!--margin-top:20px!important;-->
    <!--}-->
    <!--.u-marginTop60{-->
        <!--margin-top:60px!important;-->
    <!--}-->
    <!--.u-marginAuto{-->
        <!--margin-left:auto!important;-->
        <!--margin-right:auto!important;-->
    <!--}-->
    <!--@media screen and (max-width:767px){-->
        <!--.u-xs-marginBottom10{-->
            <!--margin-bottom:10px!important;-->
        <!--}-->
        <!--.u-xs-marginTop15{-->
            <!--margin-top:15px!important;-->
        <!--}-->
    <!--}-->
    <!--.u-paddingLeft0{-->
        <!--padding-left:0!important;-->
    <!--}-->
    <!--.u-paddingTop70{-->
        <!--padding-top:70px!important;-->
    <!--}-->
    <!--@media (max-width:991px){-->
        <!--.u-sm-paddingTop0{-->
            <!--padding-top:0!important;-->
        <!--}-->
        <!--.u-sm-paddingLeft20{-->
            <!--padding-left:20px!important;-->
        <!--}-->
    <!--}-->
    <!--@media (max-width:767px){-->
        <!--.u-xs-paddingLeft12{-->
            <!--padding-left:12px!important;-->
        <!--}-->
    <!--}-->
    <!--.u-maxWidth1000{-->
        <!--max-width:1000px!important;-->
    <!--}-->
    <!--.u-size5of12,.u-size7of12{-->
        <!--float:left!important;-->
    <!--}-->
    <!--.u-size7of12{-->
        <!--width:58.33333333%!important;-->
    <!--}-->
    <!--.u-size5of12{-->
        <!--width:41.66666667%!important;-->
    <!--}-->
    <!--@media (max-width:991px){-->
        <!--.u-sm-size12of12{-->
            <!--width:100%!important;-->
        <!--}-->
    <!--}-->
    <!--.u-textColorLight{-->
        <!--color:rgba(0,0,0,.3)!important;-->
        <!--fill:rgba(0,0,0,.3)!important;-->
    <!--}-->
    <!--.u-fontSize75{-->
        <!--font-size:77px!important;-->
    <!--}-->
    <!--.u-fontSize200{-->
        <!--font-size:202px!important;-->
    <!--}-->
    <!--@media screen and (max-width:991px){-->
        <!--.u-sm-fontSize150{-->
            <!--font-size:152px!important;-->
        <!--}-->
    <!--}-->
    <!--@media screen and (max-width:767px){-->
        <!--.u-xs-fontSize36{-->
            <!--font-size:38px!important;-->
        <!--}-->
    <!--}-->
    <!--.u-fontWeightBold{-->
        <!--font-weight:700!important;-->
        <!--font-weight:600!important;-->
    <!--}-->
    <!--.u-lineHeightTightest{-->
        <!--line-height:1!important;-->
    <!--}-->
    <!--.col,.container,.row{-->
        <!-- -webkit-box-sizing:border-box;-->
        <!--box-sizing:border-box;-->
    <!--}-->
    <!--.container{-->
        <!--padding-right:20px;-->
        <!--padding-left:20px;-->
        <!--margin-right:auto;-->
        <!--margin-left:auto;-->
    <!--}-->
    <!--.row{-->
        <!--margin-right:-20px;-->
        <!--margin-left:-20px;-->
    <!--}-->
    <!--.row:after,.row:before{-->
        <!--display:table;-->
        <!--content:" ";-->
    <!--}-->
    <!--.row:after{-->
        <!--clear:both;-->
    <!--}-->
    <!--.col{-->
        <!--position:relative;-->
        <!--min-height:1px;-->
        <!--padding-right:20px;-->
        <!--padding-left:20px;-->
    <!--}-->
    <!--.surface{-->
        <!--top:0;-->
        <!--height:100%;-->
        <!--width:100%;-->
        <!-- -webkit-box-sizing:border-box;-->
        <!--box-sizing:border-box;-->
    <!--}-->
    <!--.surface:after,.surface:before{-->
        <!--display:table;-->
        <!--content:" ";-->
    <!--}-->
    <!--.surface:after{-->
        <!--clear:both;-->
    <!--}-->
    <!--.screenContent{-->
        <!--outline:0;-->
        <!--background-color:#fff;-->
    <!--}-->
    <!--@media not print{-->
        <!--.screenContent{-->
            <!--height:100%;-->
        <!--}-->
    <!--}-->
    <!--.link&#45;&#45;underline{-->
        <!--text-decoration:underline;-->
    <!--}-->
    <!--.butterBar{-->
        <!--position:fixed;-->
        <!--left:50%;-->
        <!--pointer-events:none;-->
        <!-- -webkit-transform:translate(-50%,-100px);-->
        <!--transform:translate(-50%,-100px);-->
        <!--max-width:640px;-->
        <!--z-index:800;-->
        <!--text-align:center;-->
        <!--visibility:hidden;-->
        <!--opacity:0;-->
        <!-- -webkit-transition:visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;-->
        <!--transition:visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;-->
        <!--transition:transform .7s,visibility 0s linear .7s,opacity .7s;-->
        <!--transition:transform .7s,visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;-->
    <!--}-->
    <!--@media all and (max-width:991px){-->
        <!--.butterBar{-->
            <!--width:80%;-->
            <!--max-width:none;-->
        <!--}-->
    <!--}-->
    <!--@media all and (max-width:767px){-->
        <!--.butterBar{-->
            <!--width:90%;-->
        <!--}-->
    <!--}-->
    <!--@media print{-->
        <!--.butterBar{-->
            <!--position:absolute;-->
            <!-- -webkit-transition:none;-->
            <!--transition:none;-->
        <!--}-->
    <!--}-->
    <!--.site-main{-->
        <!--height:100%;-->
        <!--background:#fff;-->
    <!--}-->
    <!--@media print{-->
        <!--body{-->
            <!--overflow:hidden;-->
        <!--}-->
        <!--body{-->
            <!--color:#000;-->
            <!--orphans:2;-->
            <!--widows:2;-->
        <!--}-->
    <!--}-->




<!--</style>-->



<style scoped>

    /*! CSS Used from: https://cdn-static-1.medium.com/_/fp/css/main-branding-base._lSoV8jnvyEzg3I0Y9SGBw.css */
    body{margin:0;}
    section{display:block;}
    a{background-color:transparent;}
    a:active,a:hover{outline:0;}
    h1{font-size:2em;margin:.67em 0;}
    body{font-family:medium-content-sans-serif-font,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen,Ubuntu,Cantarell,"Open Sans","Helvetica Neue",sans-serif;letter-spacing:0;font-weight:400;font-style:normal;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;-moz-font-feature-settings:"liga" on;color:rgba(0,0,0,.84);font-size:20px;line-height:1.4;}
    h1,h2{
        line-height: 80px;
        font-family:medium-content-sans-serif-font,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen,Ubuntu,Cantarell,"Open Sans","Helvetica Neue",sans-serif;letter-spacing:0;font-weight:700;font-style:normal;}
    a{color:inherit;text-decoration:none;}
    a{-webkit-tap-highlight-color:transparent;}
    body{overflow-x:hidden;}
    h1,h2,p{margin:0;}
    p{margin-bottom:30px;}
    @media print{
        h2{page-break-after:avoid;page-break-inside:avoid;}
    }
    @media print and (color){
        *{-webkit-print-color-adjust:exact;print-color-adjust:exact;}
    }
    .u-marginBottom0{margin-bottom:0!important;}
    .u-marginTop20{margin-top:20px!important;}
    .u-marginTop60{margin-top:60px!important;}
    .u-marginAuto{margin-left:auto!important;margin-right:auto!important;}
    @media screen and (max-width:767px){
        .u-xs-marginBottom10{margin-bottom:10px!important;}
        .u-xs-marginTop15{margin-top:15px!important;}
    }
    .u-paddingLeft0{padding-left:0!important;}
    .u-paddingTop70{padding-top:70px!important;}
    @media (max-width:991px){
        .u-sm-paddingTop0{padding-top:0!important;}
        .u-sm-paddingLeft20{padding-left:20px!important;}
    }
    @media (max-width:767px){
        .u-xs-paddingLeft12{padding-left:12px!important;}
    }
    .u-maxWidth1000{max-width:1000px!important;}
    .u-size5of12,.u-size7of12{float:left!important;}
    .u-size7of12{
        /*width:58.33333333%!important;*/
    }
    .u-size5of12{width:41.66666667%!important;}
    @media (max-width:991px){
        .u-sm-size12of12{width:100%!important;}
    }
    .u-textColorLight{color:rgba(0,0,0,.3)!important;fill:rgba(0,0,0,.3)!important;}
    .u-fontSize75{font-size:77px!important;}
    .u-fontSize200{font-size:202px!important;}
    @media screen and (max-width:991px){
        .u-sm-fontSize150{font-size:152px!important;}
    }
    @media screen and (max-width:767px){
        .u-xs-fontSize36{font-size:38px!important;}
    }
    .u-fontWeightBold{font-weight:700!important;font-weight:600!important;}
    .u-lineHeightTightest{line-height:1!important;}
    .col,.container,.row{-webkit-box-sizing:border-box;box-sizing:border-box;}
    .container{padding-right:20px;padding-left:20px;margin-right:auto;margin-left:auto;}
    .row{margin-right:-20px;margin-left:-20px;}
    .row:after,.row:before{display:table;content:" ";}
    .row:after{clear:both;}
    .col{position:relative;min-height:1px;padding-right:20px;padding-left:20px;

        margin-top: 80px

    }
    .surface{top:0;height:100%;width:100%;-webkit-box-sizing:border-box;box-sizing:border-box;}
    .surface:after,.surface:before{display:table;content:" ";}
    .surface:after{clear:both;}
    .screenContent{outline:0;background-color:#fff;}
    @media not print{
        .screenContent{height:100%;}
    }
    .link--underline{text-decoration:underline;}
    .butterBar{position:fixed;left:50%;pointer-events:none;-webkit-transform:translate(-50%,-100px);transform:translate(-50%,-100px);max-width:640px;z-index:800;text-align:center;visibility:hidden;opacity:0;-webkit-transition:visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;transition:visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;transition:transform .7s,visibility 0s linear .7s,opacity .7s;transition:transform .7s,visibility 0s linear .7s,opacity .7s,-webkit-transform .7s;}
    @media all and (max-width:991px){
        .butterBar{width:80%;max-width:none;}
    }
    @media all and (max-width:767px){
        .butterBar{width:90%;}
    }
    @media print{
        .butterBar{position:absolute;-webkit-transition:none;transition:none;}
    }
    .site-main{height:100%;background:#fff;}
    @media print{
        body{overflow:hidden;}
        body{color:#000;orphans:2;widows:2;}
    }
    /*! CSS Used fontfaces */
    @font-face{font-family:'medium-content-sans-serif-font';font-weight:400;font-style:normal;src:url('https://glyph.medium.com/font/d8659c9/3k-4f_4h-6bt_6bv-6c3_6c5-6c7_6ca-6cb_6ce-6ch_6cj-6cl_6cn-nvnj/marat-sans-400-normal.woff') format('woff');unicode-range:U+80-9F,U+A1-2009,U+200B-2013,U+2015-2017,U+201A-201B,U+201E-2021,U+2023-2025,U+2027-10FFFF;}
    @font-face{font-family:'medium-content-sans-serif-font';font-weight:400;font-style:normal;src:url('data:font/opentype;base64,') format('woff');unicode-range:U+0-7F,U+A0,U+200A,U+2014,U+2018,U+2019,U+201C,U+201D,U+2022,U+2026;}
    @font-face{font-family:'medium-content-sans-serif-font';font-weight:600;font-style:normal;src:url('https://glyph.medium.com/font/6f4b679/3k-4f_4h-6bt_6bv-6c3_6c5-6c7_6ca-6cb_6ce-6ch_6cj-6cl_6cn-nvnj/marat-sans-600-normal.woff') format('woff');unicode-range:U+80-9F,U+A1-2009,U+200B-2013,U+2015-2017,U+201A-201B,U+201E-2021,U+2023-2025,U+2027-10FFFF;}
    @font-face{font-family:'medium-content-sans-serif-font';font-weight:600;font-style:normal;src:url('data:font/opentype;base64,') format('woff');unicode-range:U+0-7F,U+A0,U+200A,U+2014,U+2018,U+2019,U+201C,U+201D,U+2022,U+2026;}






</style>